...
class FrameStackTest < MiniTest::Test
  ...
  def test_object_can_be_proc
    @fs[:pr] = ->() { 11 }
    assert_equal @fs[:pr], 11
  end
  def test_object_is_not_a_proc
    @fs[:xx] = 'xx'
    assert_equal 'xx', @fs[:xx]
  end
  ...
  def test_globalize
    ...
    assert_equal 10, fs[:foo]
    assert_equal 10, fs.first[:foo]
  end
